window.onload = () => {
	...
	var g = Gemini.Combinatorics.PlaneGraph.fromIntCode(code);
	var boundary = { left: 0, top: 0, width: 500, height: 500 };
	var faceCenters = [];
	var pos = Gemini.Combinatorics.PlaneGraphEmbbeder.embed(
		g,
		boundary,
		faceCenters,
		new Gemini.Combinatorics.PlaneGraphEmbbederSettings(),
		null);
	document.getElementById("content").innerHTML = Gemini.Combinatorics.PlaneGraphToSvg.convert(g, pos, boundary.width, boundary.height);
};
